...
6. **Save TID:** Save the generated document as `[###]_FTID|[feature-name].md` inside the `0xcc/tids/` directory.
...
15. **Decisions from Clarifying Questions:**
    - Record each implementation clarifying question asked, the answer chosen, and its implications
    - This preserves the implementation reasoning for later sessions and for task generation
...
* **Format:** Markdown (`.md`)
* **Location:** `0xcc/tids/`
* **Filename:** `[###]_FTID|[feature-name].md` (should match corresponding PRD and TDD name patterns)
...
5. Record the clarifying questions and their answers in the "Decisions from Clarifying Questions" section
6. Ensure the TID provides sufficient detail for accurate and comprehensive task generation
7. Include specific implementation details that will translate directly into actionable tasks
8. After saving, record the document in AGENT.md: add/flip its Document Inventory entry to ✅ and update Current Status/Next Steps