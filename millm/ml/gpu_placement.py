...
#:
#: Used only for a split judged by the 20% slack (memory_utils.MEMORY_OVERHEAD_FACTOR):
#: a checkpoint whose KV cache cannot be sized from its config, and choose_gpu's
#: default rule (GGUF has its own). A split the per-card fit can size is budgeted per card instead
#: (model_loader.kv_aware_shard_rule): each card's free memory less its CUDA
#: context, cut further for a card whose layers' KV cache then does not fit, and
#: re-planned until transformers' own map fits every card (review round 5,
#: 2026-09-14). Round 4 kept this reserve in that budget because, without a
#: KV-aware cut, a whole-free-memory budget let accelerate fill a card to within a
#: layer and the fit then refused it (OLMo-2-13B and Vicuna-13B on 11,500 / 23,500).
