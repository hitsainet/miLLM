...
## Current Status
- **Phase:** ✅ **CIRCUIT RUNTIME INCREMENT CLOSED (2026-07-20)** — F12 (multi-SAE attach/serving), F13 (circuit import, slice-fallback, evidence ladder), F14 (circuit-aware OWUI dial) and F15 (circuit edge sensing) all IMPLEMENTED, each with three review rounds: **349 findings / 135 fixed**. miLLM now consumes miStudio's whole circuits arc end-to-end — import a `circuit-definition/v1`, serve it across several SAEs under one λ, dial it per request from Open WebUI with its evidence rung echoed, and watch its edges fire on live traffic — with the evidence ladder surfaced verbatim throughout and "causal" structurally impossible below rung 2. Suite **1588 backend / 255 frontend** green.
- **Recorded tech debt (settled designs, not silently dropped):** a steering epoch closing the unlocked save→generate→restore window on BOTH the circuit and Feature 10 profile paths; `CircuitSteeringEngine` extraction (one serving derivation instead of three); a `SensingRequestContext` owning the position counter, ring and event budget (three of F15's eight criticals share that root cause); moving the edge machinery out of `sae_wrapper.py`. **Two F15 criteria are NOT met and are recorded in its FTASKS:** the alone-vs-within side channel, and the ground-truth capture rate (needs a live GPU serve).
- **Methodological note worth keeping:** across the increment, every review round found a critical regression in the previous round's fix — 12 rounds, 12 for 12. The recurring shape is a fix that is correct in isolation but wrong about the system around it. R3 of F15 also showed that 14 mutation experiments found four defects that two rounds of careful reading had missed, one of which R1 had explicitly recorded as "verified clean".
- **Phase (prior):** Sensing enhancements COMPLETE (2026-07-17) — 4 goal items delivered + 3 review rounds (49 findings / 38 fixed)
...
**Increment: Circuit Runtime (BRD-MILLM-CIRCUITS-001) — ✅ CLOSED 2026-07-20. All four features (F12–F15) IMPLEMENTED + each 3-round-reviewed. Docs complete 2026-07-20 (PPRD v1.2 + PADR v1.2 + mcp-contract v1.1 + Features 12–15 chain).** Multi-SAE attach + live circuit serving (`mistudio.circuit-definition/v1`, each member through its own layer's SAE, per-layer budgets under one λ), per-layer cluster-slice fallback, circuit-aware OWUI dial, edge-level sensing; surfaces the EvidenceRung verbatim ("causal" forbidden <rung 2, unvalidated-activation ack). **VRAM spike DONE** (RTX 3090, two Gemma-2-2B SAEs): 128 MB fp16 (within <200 MB envelope) / 256 MB fp32 — attach steering weights in fp16; dtype-conditional, not a blocker. Retired the former "Multi-SAE Support" future stub (now Feature 12); future stubs renumbered 16+. Next step: execute the feature chain via `0xcc/instruct/007_generate-tasks.md` → `008_process-task-list.md` (F12 first — VRAM spike already done as task 1.0).

**Feature 12: Multi-SAE Attach & Circuit Serving** — ✅ **IMPLEMENTED + 3 REVIEW ROUNDS (2026-07-20)**. `AttachedSAEState` singleton → `(sae_id,layer)` registry (back-compat singular props); `attach_set` (referenced-only fp16 load, pre-validation, free-VRAM gate, rollback); `set_circuit_steering` (each member through ITS layer's SAE, per-layer budgets under one λ, canonical sign rule, fail-closed `SAE_SET_INCOMPLETE`, hazards surfaced never applied); routes `GET /api/saes/attachments` + `POST /api/saes/attach-set`; AttachmentPanel on SAEPage. **80 review findings, 27 fixed** (3 reproduced serving bugs). Docs 012_FPRD/FTDD/FTID/FTASKS; review record `0xcc/reviews/review_feature012_*`. Six cross-cutting items handed to F013 task 7.5.
**Feature 13: Circuit Import, Slice-Fallback & Evidence Ladder** — ✅ **IMPLEMENTED + 3 REVIEW ROUNDS (2026-07-20)**. `circuits` table (migration 011) + CircuitService (import, per-referenced-SAE compat matrix, evidence-gated activation, per-layer slice fallback through the UNCHANGED Feature 8 cluster path, lossless export); 8 `/api/circuits/*` routes; Circuits admin-UI page with the rung badge (server-rendered verbatim), unvalidated-ack gate and slice disclosure; evidence ladder mirrored byte-identically from miStudio with a copy-audit that fails the build on an overclaim. **85 review findings, 27 fixed** — incl. the slice path having never executed (R1), a circuit claiming to serve while unsteered (R2), an evidence-gate bypass across restart (R2), and a startup-reset transaction regression (R3). Docs 013_*; review record `0xcc/reviews/review_feature013_*`.
**Feature 14: Circuit-Aware OWUI Dial** — ✅ **IMPLEMENTED + 3 REVIEW ROUNDS (2026-07-20)**. Per-request whole-circuit dial (one λ scales every layer, each through its own SAE, re-derived from the AUTHORED basis so the dial is absolute); `X-miLLM-Circuit-Rung` echoed RFC 8941-structured only when a circuit is genuinely steering; `millm_dial_filter.py` → v1.4.1 "miLLM Steering Dial" (off-loop cached probe, server-verdict deferral, sanitised names, min≡off disclosure); manual + OpenAI-API reference + mcp-contract §4b. **104 review findings, 40 fixed** — the rounds compounded: R1 found the de-scaling math wrong two ways and a blocking probe; R2 found BOTH of R1's headline fixes had reintroduced what they closed (snapshot keyed off the wrong source → permanent global-state leak; the echo-parity fix landed on the λ header, not the rung header it was written for); R3 — all four perspectives independently — found R2's memo was on an `@lru_cache` SINGLETON, not a request-scoped object, so a deactivated circuit advertised `rung 2` forever. Root cause every time: parallel derivations of one fact. Docs 014_*; records `0xcc/reviews/review_feature014_*`. **§9.1 acceptance is GPU-pending** (observably-different generated text needs a live k8s serve). Deferred with a settled design: steering epoch + `CircuitSteeringEngine` (both span the Feature 10 path).
**Feature 15: Circuit Edge Sensing** — ✅ **IMPLEMENTED + 3 REVIEW ROUNDS (2026-07-20)**. Edge-scoped sensing (upstream fires, downstream partner follows within a lag window) across a circuit's SAEs via a shared `EdgeFireRing` keyed on absolute position; migration 012 + `circuits.sensing_enabled`; 6 `/api/circuit-sensing/*` routes; `circuit:sensing:event` WS; EdgeSensingPanel with **unsensable-edge reasons surfaced prominently** (so "no events" can never read as "the edge never fired"). Three refusal reasons incl. one the FPRD's edge-case list missed (`endpoint_not_a_feature`). **80 review findings, 41 fixed, 11 critical** — every round found a critical regression in the previous round's fix, and R3 caught R2 *repeating R1's exact error* (declaring a pruning mechanism and never wiring it, twice, the second time with a test named for the defect). R3's 14 mutation experiments found 4 load-bearing lines no test caught, including a WS prompt-text leak that R1 had recorded as "privacy holds — verified clean". **Two criteria NOT met and recorded as such:** the alone-vs-within side channel (BR-007/008, never implemented) and the ground-truth capture rate (§9.1, needs a live GPU serve). Docs 015_*; records `0xcc/reviews/review_feature015_*`.
