...
6. **Save TDD:** Save the generated document as `[###]_FTDD|[feature-name].md` inside the `0xcc/tdds/` directory.
...
14. **Decisions from Clarifying Questions:**
    - Record each technical clarifying question asked, the answer chosen, and its implications
    - This preserves the design reasoning for later sessions and for the downstream TID and task generation steps
...
* **Format:** Markdown (`.md`)
* **Location:** `0xcc/tdds/`
* **Filename:** `[###]_FTDD|[feature-name].md` (should match the corresponding PRD name pattern)
...
6. Ensure the TDD provides enough technical detail for the next phase (Technical Implementation Document)
7. After saving, record the document in AGENT.md: add/flip its Document Inventory entry to ✅ and update Current Status/Next Steps