...
5. **Generate ADR:** Create a comprehensive architecture decision record using the structure outlined below, including both a full documentation version and a condensed AGENT.md section.
6. **Save ADR Files:** Save both the complete document as `000_PADR|[project-name].md` in `0xcc/adrs/` and generate the AGENT.md content section.
...
The generated ADR should include the following sections in the **full documentation**, with key elements summarized in the **AGENT.md content section**:
...
### AGENT.md Content Section Structure:

The condensed section for AGENT.md should follow this format:
...
* **Format:** Two files will be generated:
  1. **Full ADR:** `000_PADR|[project-name].md` in `0xcc/adrs/` directory (complete documentation)
  2. **AGENT.md Content:** Condensed standards for inclusion in project's `AGENT.md` file

## AGENT.md Integration

The ADR will generate a **"Project Standards"** section specifically formatted for AGENT.md that includes:
...
This content should be **copied into the project's AGENT.md file** to ensure all AI interactions follow established project standards.

## Integration with Workflow

This ADR should be:
- **Created after** the Project PRD is complete
- **Content added to AGENT.md** immediately after generation
- **Followed by** generating the project foundation backlog (`007_generate-tasks.md` with the Project PRD + this ADR → `000_FTASKS|Project_Foundation.md`)
...
4. Create both a comprehensive ADR and a condensed AGENT.md section
5. Ensure the AGENT.md content captures the essential standards for AI memory
6. Include clear, actionable guidelines that can be easily referenced during development
7. Format the AGENT.md section for easy copy-paste into the project's AGENT.md file
8. Consider the project's specific requirements, timeline, and team capabilities when presenting options
9. After saving, record the document in AGENT.md: flip its Document Inventory entry to ✅ and update Current Status/Next Steps (next: the foundation backlog via `007_generate-tasks.md`)