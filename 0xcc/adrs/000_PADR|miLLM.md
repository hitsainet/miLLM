...
#### Circuit-serving engine in `ml/` vs a service collaborator
**Decision:** `CircuitSteeringEngine` lives in `millm/ml/circuit_steering.py`, constructed with an `AttachedSAEState` and nothing else
**Trade-off:** A serving concept sits outside `services/`, where the rest of circuit orchestration lives, vs a placement that reads more naturally by name
**Rationale:** The layering is already strictly one-directional — `services/` imports `ml/` (e.g. `sae_service.py:34-36`) and **no module under `ml/` imports `services/`**. Placing the engine in `ml/` therefore makes "this cannot acquire a repository or a DB session" a property enforced by import direction rather than by a comment nobody re-reads. That matters because the pressure that produced the `SAEService.__new__` bypass at `inference_service.py:743` was exactly the wish for a repository-free serving path on the request hot path; removing the pressure structurally is what retires the bypass permanently instead of relocating it. A module that cannot import a repository cannot grow a hidden second source of truth.

#### The canonical sign rule is normative, not a docstring
**Decision:** The rule — a NEGATIVE strength is already directional, so a consumer must NOT multiply it by `sign` — is recorded here as normative and stays implemented in exactly one place, `_directional_budget` (`sae_service.py:66`)
**Trade-off:** One more normative statement to keep current vs leaving the rule where the code that needs it already is
**Rationale:** The rule is load-bearing for BOTH circuits and clusters, and its failure mode is silent and inverted: double-negation turns a suppression into an amplification, so the model does the opposite of what the author asked while every surface reports success. It has existed in miLLM only as a docstring at `sae_service.py:66-76`. Feature 18 consolidates serving, and "consolidating serving" reads like an invitation to move the strength math into the new engine — which is precisely how the double-negation returns. Recording it normatively, and stating that the engine is a CALLER of `set_circuit_steering` rather than a reimplementation of it, closes that path.
...
**Rationale:** The slack grew with the weights, not with what each card needs. On 11.5 + 23.5 GB free it refused Qwen2.5-14B at FP16, whose map leaves 1,941 / 3,121 MiB after a 4k cache and the context; it accepted OLMo-2-13B, whose cuda:0 is 690 MiB short of an 8k cache; and it split a 7B FP16 model that a 3090 with 17 GB free holds (weights 14,525 MiB, 17,395 with the slack). The KV formula mirrors transformers' `DynamicCache` (layer types, sliding windows, per-layer overrides) and does not guess what it cannot size. `SHARD_RESERVE_MB` stays only for splits judged by the slack.
**Amended (review round 5, 2026-09-14):** (1) the KV cache is sized at `TRANSFORMERS_MIN_CONTEXT` or the model's own `max_position_embeddings`, whichever is shorter — requests past the latter are refused anyway, and sized at the setting OLMo-2-13B was refused at 8k for a cache no request can fill; (2) a split is budgeted per card (free less CUDA context) with the plan's need including the KV cache, and a split whose card is short is re-planned with that card's limit cut — limits only fall, the search is bounded (`FIT_REBALANCE_MAX_PASSES`), and a placement is returned only when transformers' map for exactly that placement fits every card. Measured across 2,940 plans of seven real checkpoints: no load round 4 accepted is refused, 227 it refused are accepted (138 through re-planning); (3) SAE attachment keeps each card's admitted KV cache free; (4) the KV formula is verified against transformers' own cache by generating with 14 architectures.
**Amended (hardware acceptance, 2026-09-14):** the fit counted no working memory, and it failed on the node. OLMo-2-13B FP16 was admitted on cuda:0 with 11 MiB to spare, and a 3,879-token request ran it out of memory ("Tried to allocate 76.00 MiB", 11.22 GiB allocated, 285 MiB reserved but unallocated). The Q8 split did the same. Each card now also holds a request's **working memory**: T + ⌈0.40 × (T + KV)⌉. T is the transient peak of the phases the card runs, traced by running the model's own forward pass on the meta device and following every storage, with SDPA and bitsandbytes' int8 kernels modelled from what runs on the node. The 0.40 is the caching allocator's share, calibrated by replaying traced requests through a model of torch's CUDACachingAllocator: across 90 cases the excess never passed 0.338 × (T + KV). A bitsandbytes load also counts 7% of the bfloat16 size of the weights quantized on each card, which is what the load's staging copies leave stranded (replayed: 670 / 507 MiB against the node's 653 / 444). A model that cannot be traced gets an estimate above every traced model, logged as an error. **Validated:** the same trace and allocator model reproduce the node's error to 11.21 GiB allocated and 310 MiB reserved-but-unallocated, and the three requests that succeeded afterwards to within 1% of their peaks. **Trade-off:** one meta-device forward pass per plan (about 150 ms for a 40-layer model), and a kernel model to keep in step with torch and bitsandbytes, against admitting cards that cannot serve the context they were admitted for. `TRANSFORMERS_CUDA_CONTEXT_MB` stays 500 and covers the CUDA context alone (measured at 250–400 MiB). SAE attachment keeps each card's working memory free as well as its KV cache. At the node's free memory (11,767 / 23,976 MiB), OLMo-2-13B FP16 on Auto is now re-planned to 13 / 27 layers with 230 MiB to spare on cuda:0.
